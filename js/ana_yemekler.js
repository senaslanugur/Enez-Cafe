  ...
  for(var k = 0; k < dat.prodcuts.length; k++){
    ...
    +(dat.prodcuts[k].manu_name)+'</h6>'+(dat.prodcuts[k].details)+'<p>'+(dat.prodcuts[k].price)+' TL</p></div></div><div class="card-footer p-4 pt-0 border-top-0 bg-transparent"><div class="text-center"><button type="button" class="btn btn-primary" onclick="test()">Ekle</button></div>'+
    '</div></div></div>'}
function test(){
	alert("test	")
}